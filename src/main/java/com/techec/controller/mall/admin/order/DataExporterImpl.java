package com.techec.controller.mall.admin.order;
		...
		for (String pro : proportyNames) {
			...
			for(int i=0;i<pros.length && newData != null;i++){
				String methodName = "get" + pros[i].substring(0, 1).toUpperCase()
						+ pros[i].substring(1);
				Method method = newData.getClass().getMethod(methodName);
				value = method.invoke(newData);
				newData=value;
			}
			...
			if (value == null) {
				...
				@SuppressWarnings("rawtypes")
				Map params = entity.getParamsMap().get(pro);
				if (params != null) {
	...
	private void createExcel(File file, ExportEntity entity) throws Exception {
		...
		try {
			...
			for (String string : headers) {
				...
				Label headLabel = new Label(headLableIndex, 1, entity
				...
				if (dataIndex == entity.getSizePerPage()) {
					...
					for (String string : headers) {
						...
						Label headLabel = new Label(headLableIndex, 1,
                                entity.getPro_desc_map().get(string),
								CBwcfF2);
						sheet.addCell(headLabel);
					}
				}
