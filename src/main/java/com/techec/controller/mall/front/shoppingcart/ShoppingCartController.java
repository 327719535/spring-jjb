package com.techec.controller.mall.front.shoppingcart;
...
public class ShoppingCartController extends AbstractController {
	 ...
	 */
	@ResponseBody
	@RequestMapping("/addOneToShoppingCart")
	 ...
	 */
	@ResponseBody
	@RequestMapping("/subOneToShoppingCart")
	 ...
	 */
	@ResponseBody
	@RequestMapping("/editNumeberToShoppingCart")
	 ...
	 */
	@ResponseBody
	@RequestMapping("/queryCurrentUserAllBuyShoppingCart")
	 ...
	 */
	@ResponseBody
	@RequestMapping("/delCurrentUserAllBuyShoppingCart")
	 ...
	 */
	@ResponseBody
	@RequestMapping("/delCurrentUserAllShoppingCart")
	...
	/**
	 * 删除当前登录用户所有购物车中选中的规格，如果用户未登录，则返回SystemCode.NO_LOGIN
	 * 
	 * @param request
	 * @return
	 */
	@ResponseBody
	@RequestMapping("/delCurrentUserSomeShoppingCart")
	public JsonResponse<List<ShoppingCart>> delCurrentUserSomeShoppingCart(
			String priceIds, HttpServletRequest request) {
		JsonResponse<List<ShoppingCart>> result = new JsonResponse<List<ShoppingCart>>();
		User user = SessionUtil.getUser(request);
		if (user == null || StringUtils.isBlank(user.getPhone())) {
			result.setRes(SystemCode.NO_LOGIN);
		...
		try {
			shoppingCartService.delAllShoppingCartByUserIdAndPriceIds(
					user.getUserId(), priceIds);
			result.setRes(SystemCode.SUCCESS);
		} catch (Exception e) {
			logError(request, "[admin:" + user.getNickname()
					+ ",删除当前登录用户所有准备购买的规格异常]", e);
	 ...
	 */
	@ResponseBody
	@RequestMapping("/queryCurrentUserTotalPriceAndCategory")
	public JsonResponse<TotalPriveAndCategory> queryCurrentUserTotalPriceAndCategory(
		...
		if (user != null && StringUtils.isBlank(user.getPhone()) == false) {
			 ...
			 * totalPriveAndCategory.setTotalCategory(resultList.size()); }
			 */
	...
	@ResponseBody
	@RequestMapping("/queryShoppingCartInfo")
	 ...
	 */
	@ResponseBody
	@RequestMapping("/queryShoppingCartDetailVO")
	 ...
	 */
	@ResponseBody
	@RequestMapping("/reSetIsBuyState")
